...
public static class ConsoleHelper
{
    ...
    public static void ClearLine(int y) //helper metod o bu satırı siliyor
    {
    ...
    public static async Task PrintBlinkingText(string message, Point point, int delay = 500, CancellationToken token = default)
    {
        while (true) 
        {
            ResetCursorPosition(point.X, point.Y);//ekrana sürekli yazdırıyo
            Console.Write(message);
            await Task.Delay(delay, token);

            ResetCursorPosition(point.X, point.Y);//temizliyor
            Console.Write(new string(' ', message.Length));

            await Task.Delay(delay, token); //bi daha bastırıyo
        }
    }
